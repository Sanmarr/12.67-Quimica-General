...
#table(
  ...
  [Iónica], [Atracción electrostática entre iones de carga opuesta. Se da entre un #highlight[metal (*cation*) y no-metal (*anion*)] (gran diferencia de EN). A temperatura ambiente, son solidos que forman redes cristalinas \ Ej: $"Na"^+ + "Cl"^- arrow.r "NaCl"$],
  [Covalente], [Los átomos comparten pares de electrones. Se produce #highlight[entre no metales] (incluyendo el hidrógeno) con baja diferencia de EN. *Simple*: Se comparte 1 par de electrones ($H-H$, $"Cl"-"Cl"$) \ *Doble*: Se comparten 2 pares de electrones ($O=O$) \ *Triple*: Se comparten 3 pares de electrones ($N equiv N$) \ *Dativa* (Coordinada): Los dos electrones del par compartido son aportados por uno solo de los átomos ($"SO"_2$)],
  [Metálica], [Atracción entre cationes metálicos y una _nube_ de electrones libres.]
)
...
#figure(
  table(
    ...
    [G1], [$n s^1 = 1e^-$], [$M bullet$], [$M^+$], [$M^+$], [$"Na"^+$],
    
    [G2], [$n s^2 = 2e^-$], [$bullet M bullet$], [$M^(2+)$], [$M^(2+)$], [$"Ca"^(2+)$],
    
    [G13], [$n s^2 n p^1 = 3e^-$], [$bullet M:$], [$M^(3+)$], [$M^(3+)$], [$"Al"^(3+)$],
    
    [G14], [$n s^2 n p^2 = 4e^-$], [$bullet X bullet \ bullet bullet$], [No forma un ión simple estable], [--], [C, $"Si"$],
    
    [G15], [$n s^2 n p^3 = 5e^-$], [$bullet X: \ bullet bullet$], [$X^(3-)$], [$[ :X: ]^(3-)$], [$[ :N: ]^(3-)$],
    
    [G16], [$n s^2 n p^4 = 6e^-$], [$:X: \ bullet bullet$], [$X^(2-)$], [$[ :X: ]^(2-)$], [$[ :O: ]^(2-)$],
    
    [G17], [$n s^2 n p^5 = 7e^-$], [$:X: \ :$], [$X^-$], [$[ :X: ]^-$], [$[ :"Cl": ]^-$]
  ),
  caption: [Iones estables y símbolos de Lewis de los elementos representativos.]
)
...
//  caption: [Representación de estructuras de Lewis para diversos grupos]
//) <fig:lewis>